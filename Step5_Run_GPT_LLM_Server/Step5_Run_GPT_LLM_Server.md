...
- [1. 完全にローカルで完結した AI を実現することの意義](#1-完全にローカルで完結した-ai-を実現することの意義)
  - [1.1. ローカル AI の、クラウド型 AI に対するメリット](#11-ローカル-ai-のクラウド型-ai-に対するメリット)
  - [1.2. ローカル AI の、クラウド型 AI に対するデメリット](#12-ローカル-ai-のクラウド型-ai-に対するデメリット)
...
- [6. 使いやすい Web 型の翻訳アプリを作ってみよう](#6-使いやすい-web-型の翻訳アプリを作ってみよう)
  ...
  - [6.2. 作成された Python コードの安全性を検証](#62-作成された-python-コードの安全性を検証)
  - [6.3. 作成された翻訳アプリのプログラムを実行](#63-作成された翻訳アプリのプログラムを実行)
    - [6.3.1. この翻訳アプリの仕組み](#631-この翻訳アプリの仕組み)
  - [6.4. 翻訳アプリの Python 用プログラムを Copilot を用いて作成することは安全なのか](#64-翻訳アプリの-python-用プログラムを-copilot-を用いて作成することは安全なのか)
...
## 1.1. ローカル AI の、クラウド型 AI に対するメリット
...
4. **挙動が不変である (安定性の実現)。** 一度 AI マシンを組み立て、AI モデルデータをダウンロードしたならば、そのモデルデータで AI を稼働させる限り、挙動は不変である。たとえば、本講義の時点 (2026 年) で、本講義の内容を用いて AI マシンを構築したとする。その手順をメモしておき、必要なモデルデータも保持しておけば、2030 年になったときも、2040 年になったときも、2026 年と全く同じ挙動で AI を動作させることができる。この動作の不変性は、たとえば、重要な業務システムにおける自作プログラムの一部に AI を組み込む場合に、極めて重要である。
    ...
    自作の業務システム等のプログラムから AI を呼び出す場合を考えてみよう。たとえば、ある営業戦略ツールで、顧客ごとの性質を分析して数値化し、顧客ごとに適切なセールス戦略を計算する業務システムに AI を利用したとする。ある時のあるバージョンの ChatGPT の API を呼び出したときに、ちょうど良く動作していたのに、ChatGPT の API のバージョンが上がったり、あるいは挙動がマイナー変化してしまっただけで- [1. 完全にローカルで完結した AI を実現することの意義](#1-完全にローカルで完結した-ai-を実現することの意義)
- [1. 完全にローカルで完結した AI を実現することの意義](#1-完全にローカルで完結した-ai-を実現することの意義)
  - [1.1. ローカル AI の、クラウド型 AI に対するメリット](#11-ローカル-ai-のクラウド型-ai-に対するメリット)
  - [1.2. ローカル AI の、クラウド型 AI に対するデメリット](#12-ローカル-ai-のクラウド型-ai-に対するデメリット)
- [2. OpenAI gpt-oss-120b とは](#2-openai-gpt-oss-120b-とは)
...
## 1.2. ローカル AI の、クラウド型 AI に対するデメリット

ローカル AI のデメリットは、メリットの裏返しである。何らかの利益を得ようとする場合は、多くの場合、それに伴う不利益を受忍する必要がある。

1. **ある程度高性能な物理マシンと GPU が手元で必要となる。** 当然のことであるが、ローカル AI を動作させるためには、たとえば本講義で貸し出しているような機材 (1 台 約 40 万円程度) が少なくとも 1 台必要になる。
2. **たくさんの処理をさせると、それに応じて電気代がかかる。** ローカル AI は、CPU や GPU を稼働させるので、電力をそれなりに消費する。ただ、本講義で貸し出している機材は、かなり低消費電力であり、仮に家庭のコンセントでこれを動作させて 24 時間自作プログラムから呼び出し続けても、クラウド型 AI の API 料金の累積よりは月額コストが安価になる可能性が高い。
3. **性能・新機能への追従は、自ら行なう必要がある。** ローカル AI は、AI モデルデータを取得した時点の性能で固定化される。より良い性能のローカル AI が出た場合は、自らそれをダウンロードし、現在利用しているものを新しいもので置き換える必要がある。
4. **手元の GPU の速度が遅く、メモリサイズが比較的小さい場合、性能はクラウド型 AI と比較して、一定程度低くなる。** クラウド型 AI は、きわめてメモリサイズの大きい高額な高速 GPU を複数連結して、巨大なモデルデータをロードしており、高速に演算が可能である。これと比較して、手元の GPU は、たいていの場合、速度が遅く、メモリサイズが小さい。このように、ローカル AI の性能は、クラウド型よりも低いことが多い。
5. **構築・利用にあたり、知恵やノウハウや技術力の蓄積が必要となる。** ローカル AI をうまく動作させようと熱心に試行錯誤する場合、ローカル AI だけでなく、コンピュータやインターネット、セキュリティに関する、基本的・基礎的な知識を自然に向上させる必要がある。これは短期的には時間損失であるが、長期的にみると投資であるともいえる。知恵やノウハウや技術力の蓄積は、技術者または経営企画者としての市場価値に直結するので (生産資本のようなものである)、これは、デメリットとはいえないかも知れない。
...
## 6.2. 作成された Python コードの安全性を検証

Copilot のような生成 AI (特に、クラウド型のオンラインサービス型の生成 AI) では、AI の学習の具合や、あるいは悪意のある第三者の為業により、生成されたコードに、有害なコードが紛れ込むリスクがある。

そこで、生成 AI が作成したコードは、実行前に、安全性の検証を行なう必要がある。

安全性の検証は、次の 2 つの方法のいずれか、あるいは両方で行なう。

1. 人間がコードを実際に読んでみる。たとえば、今回は Python のプログラムコードなので、とても難易度が低く、人間でも読んで理解することはそれなりに可能である。
2. いくつかの生成 AI (オンライン型でも、ローカル型でもよい) に対して、生成されたコードとともに、そのコードの危険な部分を指摘するような依頼をプロンプトで作成し、これを実行する。

上記の手順において、「1.」の場合はもちろん、「2.」の場合も、コンピュータのソフトウェアプログラミングや通信やセキュリティに関する基本的・基礎的な知識が必要となる。「2.」の場合、AI に対する聞き方を決めたり、AI の応答内容を吟味したりする際に、これらの知識が必要となる。AI は、真に危険でないコードを「危険」であると言ってきたり、あるいは、リスクが高いコードを「リスクは低い」と言ってきたりする可能性があるためである。


## 6.3. 作成された翻訳アプリのプログラムを実行

Copilot によって作成された Python プログラムを、上記のとおり、一応の安全検査を行なった後に、自身のサーバ上にファイルとして保存し、これを実行してみよう。
...
## 6.4. 翻訳アプリの Python 用プログラムを Copilot を用いて作成することは安全なのか
