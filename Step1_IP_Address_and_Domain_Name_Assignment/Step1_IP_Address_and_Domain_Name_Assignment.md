**【ステップ 1. グローバル IP アドレスとサブドメインの割り当てを受けよう】**

2026/06/01 登 初版作成

- [1. 概説](#1-概説)
  - [1.1. 大学において学生がグローバル IPv4 アドレスを利用できることの重要性](#11-大学において学生がグローバル-ipv4-アドレスを利用できることの重要性)
  - [1.2. 大学サブドメインを学生が研究目的で取得できることの重要性](#12-大学サブドメインを学生が研究目的で取得できることの重要性)
- [2. 本講義用のグローバル IPv4 アドレスとドメイン名の概要](#2-本講義用のグローバル-ipv4-アドレスとドメイン名の概要)
  - [2.1. 本講義用のグローバル IPv4 アドレス](#21-本講義用のグローバル-ipv4-アドレス)
  - [2.2. 本講義用のドメイン名](#22-本講義用のドメイン名)
- [3. どのように割り当てが管理されているのか](#3-どのように割り当てが管理されているのか)
  - [3.1. IPv4 アドレスの割り当て管理方法](#31-ipv4-アドレスの割り当て管理方法)
  - [3.2. start.coins.tsukuba.ac.jp のサブドメインの割り当て管理方法](#32-startcoinstsukubaacjp-のサブドメインの割り当て管理方法)
- [4. 実際に、グローバル IPv4 アドレスとサブドメイン名の割り当てを受けてみよう](#4-実際にグローバル-ipv4-アドレスとサブドメイン名の割り当てを受けてみよう)
  - [`ZoneDef.config` の編集方法](#zonedefconfig-の編集方法)


# 1. 概説

## 1.1. 大学において学生がグローバル IPv4 アドレスを利用できることの重要性
コンピュータに関する基本的・基礎的な知見やセキュリティリテラシを修得するためには、サーバを運用する必要がある。そのために、グローバル IPv4 アドレスを少なくとも 1 個割り当てを受けることは、非常に効果的である。

さらに、現在日本においては、若手人材が、デジタル基盤領域 (クラウドコンピューティング、OS、ネットワーク、仮想化、セキュリティ、AI 等) の技術を研究し、国際競争力のある新たなシステムを作り、多様なプラットフォーマーが形成されることが、社会的に求められている。そのためには、大学の学生の方々は、皆、グローバル IPv4 アドレスをいくつも割り当てを受け、これを自ら取扱い、自作基盤システムにつなぎこむことが必要である。
...
筑波大学でも、将来は、`tsukuba.ac.jp` の直下に、たとえば `google.tsukuba.ac.jp` というようなサブドメインを学生が研究目的で取得することができるようにすることが、大きな価値を創出するための秘訣である可能性は高い。しかし、現在は、その仕組みはない。そこで、本講義では、より下位のサブドメインではあるが、`start.coins.tsukuba.ac.jp` という `tsukuba.ac.jp` の 2 階層下のドメインを用意し、その下に `任意の文字列.start.coins.tsukuba.ac.jp` というさらなる独自のサブドメインを受講者が自分のために自由に割り当てることができるようにして、できるだけ米国の優良大学の環境に近い良好環境の実現を目指すことにした。
...
coins.tsukuba.ac.jp
```
というものがある。この `coins.tsukuba.ac.jp` は、情報科学類の計算機運用委員会が管理している。情報科学類の計算機運用委員長に近日就任した鹿野氏は、さらなるサブドメインである
```
...
# 4. 実際に、グローバル IPv4 アドレスとサブドメイン名の割り当てを受けてみよう
...
## `ZoneDef.config` の編集方法
`ZoneDef.config` ファイルには、以下のような編集可能部分がある。

```
### ==================================
### ↓↓↓↓↓ ここから編集 ↓↓↓↓↓

## ★ 「130.158.230.1」の「登 大遊」(サンプル) の項目の数行を真似して、コピー＆ペーストすればよい。
...
# 学籍番号: 202540353
# 使用開始: 2026/05/31
# 連絡先メールアドレス: sxxxxxxx@u.tsukuba.ac.jp xxxxx@dnobori.jp
130.158.230.1       dnobori.start.coins.tsukuba.ac.jp
130.158.230.1       dnobori-app1.start.coins.tsukuba.ac.jp
130.158.230.1       dnobori-app2.start.coins.tsukuba.ac.jp
130.158.230.1       dnobori-app3.start.coins.tsukuba.ac.jp
130.158.230.1       dnobori-app4.start.coins.tsukuba.ac.jp
...
上記の意味は、氏名「登 大遊」という受講者 (サンプル) が、
- `130.158.230.1` という IPv4 アドレスの割り当てを受けた (自己宣言した)。
- `dnobori.start.coins.tsukuba.ac.jp` という、 `dnobori` を基本ハンドルネーム (ここで、`.start.coins.tsukuba.ac.jp` のドメインのサブドメイン用のラベル = ハンドルネームという) としたサブドメインを利用することにした。
- `dnobori.start.coins.tsukuba.ac.jp` の `dnobori` のほかに、 `dnobori-app1`、`dnobori-app2`、`dnobori-app3`、`dnobori-app4` を `.start.coins.tsukuba.ac.jp` のドメインのサブドメイン用のラベルとして登録した。
ということを示している。

そこで、上記のサンプル表記の数行の部分全体を、すべてコピーペーストした上で、`130.158.230.★` の `★` の部分を、他の人と重複しない数字にし、かつ、`dnobori` という文字列の部分を、自分の好きな (他の人と重複しない) ハンドルネームに変更するとよい。

これで、`130.158.230.★` という IPv4 アドレスの割り当てを自己宣言したことになる。

注意点としては、自分のハンドルネームが `◎` の場合、
- `◎.start.coins.tsukuba.ac.jp`  
のみでなく、
...
の追加の 4 個のドメインラベルを含んだサブドメイン名を登録する必要がある点がある。後日実施する AI サービス構築時に、クライアントの Web ブラウザから、この `app1` から `app4` までの識別子を含むドメインラベルを指定することにより、異なる AI サービスにそれぞれ接続できるようにするためである。



そして、`ZoneDef.config` ファイルを上書き保存すると、直ちに (おおむね 1 秒程度で)、当該サブドメインは、インターネットから DNS という仕組みで名前解決できるようになる。


DNS の知識をお持ちの受講者は、手元のパソコン (任意のインターネット接続) で、nslookup コマンドまたは dig コマンド等を用いて、`◎.start.coins.tsukuba.ac.jp` (登録したドメイン名) を名前解決してみるとよい。`ZoneDef.config` ファイルを上書き保存してわずか数秒後に、大学のサブドメインで自分の好きな名前の名前解決ができるようになるのは、とても面白い体験である。
